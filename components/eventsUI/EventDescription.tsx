...
type EventDescriptionProps = {
  ...
  export const EventDescription: React.FC<EventDescriptionProps> = ({
    ...
    // Extend sections to include an array of ISectionFile
    const [sections, setSections] = useState<
      (IEventSection & { files: ISectionFile[] })[]
    >(
      initialSections.length
        ? initialSections.map((s) => ({ ...s, files: [] }))
        : [
            {
              id: 1,
              eventId: event.id,
    ...
    const createFileUrl = (fileObj: ISectionFile) => {
      ...
      try {
        if (fileObj.dataBytes instanceof Buffer) {
          ...
          console.error('Formato de dataBytes no soportado:', typeof fileObj.dataBytes);
        ...
        console.error('Error al crear blob:', e, 'Tipo de datos:', typeof fileObj.dataBytes);
              ...
              {editingSectionId === sec.id ? (
                ...
                <div className="flex items-center" onClick={(e) => e.stopPropagation()}>
                  <input
                    ...
                    onKeyPress={(e) => handleNameInputKeyPress(e, sec.id)}
